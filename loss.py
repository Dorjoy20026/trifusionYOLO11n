...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):
    ...
    # Calculate center distance and width-height distance
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16, mpdiou=False, siou=False, eiou=False, shapeiou=False, giou=False, diou=False,
                 nwd_loss=False, iou_ratio=0.5, enmpdiou=False,ensiou=False):
        ...
        self.siou = siou
        self.eiou = eiou
        self.shapeiou = shapeiou
        self.giou = giou
        self.diou = diou
        self.nwd_loss = nwd_loss
        self.iou_ratio = iou_ratio
        self.enmpdiou = enmpdiou
        self.ensiou = ensiou

    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        """IoU loss."""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        if self.mpdiou == "True" or self.mpdiou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, MPDIoU=True)
        elif self.siou == "True" or self.siou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, SIoU=True)
        elif self.eiou == "True" or self.eiou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, EIoU=True)
        elif self.shapeiou == "True" or self.shapeiou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, ShapeIoU=True)
        elif self.giou == "True" or self.giou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, GIoU=True)
        elif self.diou == "True" or self.diou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, DIoU=True)
        elif self.enmpdiou == "True" or self.enmpdiou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, EnMPDIoU=True)

        elif self.ensiou == "True" or self.ensiou==True:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, EnSIoU=True)
        else:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)

        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum

        if self.nwd_loss == "True" or self.nwd_loss == True:
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.siou = self.hyp.siou
        self.eiou = self.hyp.eiou
        self.shapeiou = self.hyp.shapeiou
        self.giou = self.hyp.giou
        self.diou = self.hyp.diou
        self.enmpdiou = self.hyp.enmpdiou
        self.ensiou = self.hyp.ensiou
        self.assigner = TaskAlignedAssigner(topk=tal_topk, num_classes=self.nc, alpha=0.5, beta=6.0)
        self.bbox_loss = BboxLoss(m.reg_max, mpdiou=self.mpdiou, siou=self.siou, eiou=self.eiou, shapeiou=self.shapeiou,
                                  giou=self.giou, diou=self.diou, nwd_loss=self.nwdloss, iou_ratio=self.iou_ratio,
                                  enmpdiou=self.enmpdiou,ensiou=self.ensiou).to(device)
