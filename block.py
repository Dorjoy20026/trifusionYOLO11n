...
__all__ = (
    ...
    "Attention",
    "PSA",
    "SCDown",'MCALayer','SimAM','GAMAttention','SE','CoordAtt','ECA','C2f_DCNv3','EnhancedMCALayer'
)
...
class DynamicContextAwareness(nn.Module):
    def __init__(self, channels):
        super(DynamicContextAwareness, self).__init__()
    ...
    def forward(self, x):
        # Generate three types of context information
        context_local = self.depthwise(x)
        context_global = torch.mean(x, dim=[2, 3], keepdim=True).expand_as(x)
        context_channel = self.pointwise(x)

        # Generate dynamic weights
        weights = self.sigmoid(self.weight_gen(x))

        # Fuse different context information
        out = weights[:, 0:1, :, :] * context_local + \
              weights[:, 1:2, :, :] * context_global + \
              weights[:, 2:3, :, :] * context_channel

        return out

# Modify MCALayer to include new DCA module
class EnhancedMCALayer(nn.Module):
    def __init__(self, inp, no_spatial=True):
        super(EnhancedMCALayer, self).__init__()
        self.mca = MCALayer(inp, no_spatial)
        self.dca = DynamicContextAwareness(inp)
