...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, MPDIoU=False, SIoU=False, EIoU=False,
             ShapeIoU=False,EnMPDIoU=False,EnSIoU=False, EnEIOU=False,alpha=1, scale=0.0, eps=1e-7):
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or MPDIoU or SIoU or EIoU or ShapeIoU or EnMPDIoU or EnSIoU or EnEIOU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or MPDIoU or SIoU or EIoU or ShapeIoU or EnMPDIoU or EnSIoU or EnEIOU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                center_distance = ((b2_x1 + b2_x2) / 2 - (b1_x1 + b1_x2) / 2) ** 2 + \
                                  ((b2_y1 + b2_y2) / 2 - (b1_y1 + b1_y2) / 2) ** 2

                # Add a small smoothing factor to prevent division by zero
                epsilon = 1e-7

                # Use smooth L1 loss instead of square loss
                smooth_l1 = lambda x: torch.where(x < 1, 0.5 * x ** 2, x - 0.5)

                penalty = (smooth_l1(d12 / (sq_sum + epsilon)) +
                           smooth_l1(d22 / (sq_sum + epsilon)) +
                           smooth_l1(center_distance / (sq_sum + epsilon)))
                return iou - penalty
